...
def create_network_one_pass(hr_frames, hr_first_frame_seg, c_tm1, h_tm1, c_tm1_lr, h_tm1_lr, use_gt, gt_crop,
    ...
    crop_to_use = convert_crop(crop_to_use)
...
def create_network(x_input, y_segmentation, f_tm1, f_tm1_lr, use_gt_seg, use_gt_crop, gt_crop1, gt_crop2):
    ...
    network_outs1 = create_network_one_pass(x_input[:, :config.n_frames], y_segmentation[:, 0], c_tm1, h_tm1, c_tm1_lr,
                                            h_tm1_lr, use_gt_crop, gt_crop1, coords_to_add, print_layers=True)

    hr_seg1, hr_seg_sig1, c_t, h_t, c_t_lr, h_t_lr, pred_crop1, fin_caps = network_outs1

    # reuse variables
    tf.get_variable_scope().reuse_variables()

    first_seg2 = tf.cond(use_gt_seg, lambda: y_segmentation[:, config.n_frames - 1], lambda: hr_seg_sig1[:, -1])

    network_outs2 = create_network_one_pass(x_input[:, config.n_frames-1:], first_seg2, c_t, h_t, c_t_lr, h_t_lr,
                                            use_gt_crop, gt_crop2, coords_to_add, print_layers=False)

    hr_seg2, hr_seg_sig2, c_t2, h_t2, c_t2_lr, h_t2_lr, pred_crop2, _ = network_outs2

    fin_seg = tf.concat([hr_seg1, hr_seg2[:, 1:]], axis=1)
    fin_seg_sig = tf.concat([hr_seg_sig1, hr_seg_sig2[:, 1:]], axis=1)

    return fin_seg, fin_seg_sig, fin_caps, tf.concat([c_t2, h_t2], axis=-1), pred_crop1, pred_crop2
